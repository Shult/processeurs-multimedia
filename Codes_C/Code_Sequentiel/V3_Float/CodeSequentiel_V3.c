...
int main(int argc, char **argv) {
    ...
    __m128 SSEPixelIn = _mm_set1_ps((float) 0.0);
    __m128 SSEPixelMin = _mm_set1_ps((float) 0.0);
    __m128 SSEEtalement = _mm_set1_ps((float) 0.0);
	__m128 SSEPixelOut = _mm_set1_ps((float) 0.0);
...
InitClock;
	...
	CALLOC(image, TailleImage, float);
	CALLOC(resultat, TailleImage, float);
	...
	SSEEtalement = _mm_set1_ps(ETALEMENT);
	SSEPixelMin = _mm_set1_ps((float) - LE_MIN);

	//SEELeMinEtalement = _mm_set1_ps((float) - LE_MIN * ETALEMENT);
...
ClockStart;
	...
	for (i = 0 ; i < TailleImage ; i = i+4) {
		SSEPixelIn = _mm_loadu_ps(&image[i]);
		SSEPixelOut = _mm_add_ps(SSEPixelIn, SSEPixelMin);
		SSEPixelOut = _mm_mul_ps(SSEPixelOut, SSEEtalement);

		_mm_storeu_ps(&resultat[i], SSEPixelOut);
	}
