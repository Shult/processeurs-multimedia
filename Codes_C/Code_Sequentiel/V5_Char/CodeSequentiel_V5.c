...
int main(int argc, char **argv) {
    ...
    __m128i SSEPixelIn = _mm_set1_epi8(0);
    __m128i SSEPixelMin = _mm_set1_epi8(0);
    __m128i SSEEtalement = _mm_set1_epi16(0);
	__m128i SSEPixelOut = _mm_set1_epi8(0);

	__m128i SSEEtalementMIN = _mm_set1_epi8(0);
	// =================================s
...
InitClock;
	...
	CALLOC(image, TailleImage, unsigned char);
	CALLOC(resultat, TailleImage, unsigned char);
	...
	while (! feof(Src)) {
		n = fscanf(Src,"%d",&P);
		// =========== MODIF ===============
		image[x] = (unsigned char)P;	// Cast en char
		// =================================
		x ++;
	...
	SSEPixelMin = _mm_set1_epi8((unsigned char) - LE_MIN);
...
ClockStart;
	...
	for (i = 0; i < TailleImage; i += 2) {  // traiter 2 pixels à la fois
		memcpy(&SSEPixelIn, &image[i], sizeof(SSEPixelIn));

        SSEPixelOut = _mm_adds_epi8(SSEPixelIn, SSEPixelMin);  // addition 
        SSEPixelOut = _mm_mullo_epi16(SSEPixelOut, SSEEtalement);  // multiplication
